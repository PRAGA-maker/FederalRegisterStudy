"""
xAI Grok response tracker for agency response tracking.

Drop-in replacement for OpenAIResponseTracker (Tier 1 only) using
xAI's OpenAI-compatible API with web search and structured outputs.

Uses grok-4-1-fast-reasoning by default with
``client.beta.chat.completions.parse()`` for structured JSON output
and ``tools=[{"type": "web_search"}]`` for web search.

Example:
    >>> from stratification_scripts.xai_response_client import XAIResponseTracker
    >>> tracker = XAIResponseTracker(api_key="xai-...")
...
class XAIResponseTracker:
    """
    Async-compatible xAI client for tracking agency responses.

    Uses grok-4-1-fast-reasoning with the OpenAI-compatible API for
    structured output and web search grounding. Sync calls are wrapped
    in asyncio.to_thread() for async compatibility with the existing pipeline.
    ...
    def __init__(
        ...
        logger.debug(
            f"Initialized XAIResponseTracker model={model} search={enable_search}"
        )
    ...
    def _is_retryable_error(self, e: Exception) -> bool:
        ...
        if "500" in msg or "502" in msg or "503" in msg or "504" in msg:
            return True
        if "400" in msg or "422" in msg or "invalid" in msg or "bad request" in msg:
            return False
    ...
    def _track_response_sync(
        ...
        Uses xAI's OpenAI-compatible API with web search and structured output.
        """
        max_chars = 20000
        ...
        for attempt in range(self.max_retries):
            try:
                logger.debug(
                    f"xAI API call attempt {attempt + 1}/{self.max_retries} "
                    f"for comment_id={comment_id}, model={self.model}"
                )

                # Use the Responses API which natively supports web_search tool
                ...
                if response.output_text:
                    try:
                        ...
                        logger.warning(
                            f"xAI: output_parsed=None, manual parse failed for {comment_id}: {parse_err}"
                        )

                # Empty response
                logger.warning(f"xAI: empty response for {comment_id}")
                return (
                    comment_id,
                    AgencyResponse(
                ...
                logger.error(
                    f"xAI API error for {comment_id} "
                ...
                if attempt >= self.max_retries - 1 or not retryable:
                    logger.warning(
                        f"xAI call failed (retryable={retryable}) after attempt {attempt + 1}: "
    ...
    async def track_batch(
        ...
        Args:
            ...
            max_concurrency: Maximum concurrent API calls (default: 50)
        ...
        for task in tqdm(
            ...
            total=len(tasks),
            desc="Tracking responses (xAI Grok)",
        ):
