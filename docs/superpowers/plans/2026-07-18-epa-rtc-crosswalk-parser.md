...
- **L4. Resolution order: exact → alias → unique-substring → unresolved.** Alias table holds only high-confidence expansions (PFAS, DBPs); unique-substring backstops; ambiguous/absent → flagged. Rejected multi-match substring (ambiguous) and edit-distance fuzzy (guessy). Reopen on labeled miss patterns. **Observed on the real doc (219 refs, 211 resolved = 96%): 8 unresolved —** `1,4-Dioxane` ×4 (genuinely out-of-list, correct to flag), `Other Drinking Water Programs (RegDet; UCMR)` ×2 (parenthetical-suffix near-miss of "Other Drinking Water Programs"), `Chemical Groups` ×1 (probable "Contaminant Groups", not force-matched — needs a semantic alias), `the Final CCL 5 Chemical Technical Support Document` ×1 (a doc reference, near "Chemical Technical Support Documents"). **Deliberately not fixed now:** stripping trailing parentheticals or adding the "Chemical Groups"→"Contaminant Groups" alias are gold-set-informed refinements, not in-the-moment guesses. This is the concrete reopener evidence for L3.
...
- **LA. Exhibit-2 parse is POSITIONAL, not heuristic (Task 5, revised during impl).** Inspecting the real extraction showed fitz emits fixed-position cells per row: `<int>` / `<doc-id>` / First / Last / Org(1+ wrapped lines), with blank name cells rendered as `' '` lines — so first=block[0], last=block[1], org=join(block[2:]). This is simpler and more robust than the plan's original "disambiguation heuristic" guess, which is hereby dropped. Rows anchored on the (int→doc-id) pair so prose Document IDs aren't captured; region bounded `[Exhibit 2 header .. following Section 2 marker]` (the marker also appears in the TOC — search after the header). Verified: all 54 real rows parse, correct names/orgs, no empty orgs. Reopen if a future row lacks the First/Last cells entirely.
...
- **L8. Derived outputs gitignored, not committed (decided during impl).** The whole `rtc/` tree is gitignored: source PDF bytes (freeze convention) AND the `rtc/<slug>/` crosswalk outputs (a 1.5 MB regenerable jsonl + friends). Behavior is pinned by the code + the committed 15 KB text fixture; provenance by each run's `parse_manifest.json` (sha256 + counts). Rejected committing outputs (bloat, and `*.csv` would hit the repo's LFS filter). Contrast goldset, which DOES commit its outputs because they are the human-labeled instrument, not a regenerable parse. Reopen if a downstream consumer needs a checked-in crosswalk snapshot (commit that one run explicitly, `.txt`/`.jsonl` to dodge the csv-LFS rule).

## Build status: COMPLETE (2026-07-18)

All 11 tasks implemented TDD, 13 commits (`18db46b`→`65de79b`). Acceptance met: CCL5 PDF parses end-to-end (54 commenters / 114 comments / 22 topics / 8 honestly-flagged unresolved refs); full repo suite 96 passed. No pipeline wiring, no validation claims. **Walkthrough banked** (see project log) — adds to the goldset ledger review.
